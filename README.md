...
2. Create a service account for the Streamlit dashboard reader.
...
For a folder in **My Drive**, use OAuth user upload mode for the local pipeline. Google Drive service accounts do not have storage quota and cannot create new files in a user's My Drive folder, even when the folder is shared with them. The service account can still be used by Streamlit to read the uploaded workbook.

For a Google Workspace **Shared Drive**, service-account upload mode can be used if the service account has the required Shared Drive permissions.
...
`GDRIVE_ENABLED=false` is the default. When disabled, no Drive service is built and the backend pipeline continues to work locally. `GDRIVE_AUTH_MODE=service_account` uses the service-account JSON content/path and is appropriate for Shared Drive uploads. `GDRIVE_AUTH_MODE=oauth_user` uses a local OAuth Desktop client and writes a Drive upload token to `GDRIVE_OAUTH_TOKEN_PATH`; the first run opens a browser consent flow. When `GDRIVE_FAIL_ON_UPLOAD_ERROR=false`, report generation remains successful even if Drive upload fails; a Drive upload audit is written under the run folder. Set `GDRIVE_FAIL_ON_UPLOAD_ERROR=true` only when the run must fail on upload errors.
...
| `GDRIVE_STORAGE_QUOTA_EXCEEDED` | Service account is trying to create files in My Drive | Use `GDRIVE_AUTH_MODE=oauth_user`, or move the folder to a Shared Drive. |
